...
//------------------------------------------------------------------------------------	 check for search
		...
		// then we are filtering
			...
			if (!empty($filtercol)) { // for each of the filter columns that are not field filters
				foreach ($filtercol as $fcol => $value) {
					...
					//if (amr_debug()) {echo '<hr>Admin only debug: Got '.count($lines).' lines left - '.'Apply filters for field "'.$fcol. '" against... '; var_dump($value); } //***
	...
	//if (WP_DEBUG) echo '<br />Lines mid filtercol '.count($lines);
					...
					//if (amr_debug()) echo '<br />Admin only debug: Records found that meet criteria: '.count($lines);
				}
